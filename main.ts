...
export async function createDispenser(options: DispenserOptions): Promise<IDispenser> {
	...
	switch (dispenserType) {
		...
		case 'TCS3000':
			...
			// xon: stop sending when the printer says XOFF (buffer full) and resume on XON. A printer
			// that never sends them behaves as before; unlike rtscts, this cannot block an unwired port.
			const TCS3000printerPort = new SerialPort({ path: TCS3000PrinterPath, baudRate: printer.baudRate || 9600, xon: true });
		...
		case 'GateX':
			...
			const printerPort = new SerialPort({ path: GateXPrinterPath, baudRate: printer.baudRate || 9600, xon: true });
